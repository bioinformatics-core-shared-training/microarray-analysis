...
The `genefilter` package in Bioconductor provides several methods to assist in the filtering of microarray data based on a lack of annotation or low variation. However, in order to use the package we first have to re-set the value of the annotation currently stored with the object. 
...
## Exercise
- Use the `nsFilter` function to filter the `ExpressionSet` object `mydata`
- How many probes remain after filtering?
    + does it seem a bit harsh? what options are available for customising the filter
...
To produce a boxplot of the gene expression of a particular gene as a function of Gender we use:-
...
The boxplot is produce for the first probe only. We can repeat for other probes by altering the index of `subset`. The `gender` vector is the same for every line of code.
