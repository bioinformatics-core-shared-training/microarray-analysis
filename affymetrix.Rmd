...
The first stage is to read the *targets* file. In Bioconductor we often use a targets to define the files corresponding to the raw data for the experiment and which sample groups they belong to. This sample group information is propogated through the analysis and incorporated in the quality assessment and eventually differential expression.
...
We can look at the images of the array surface. Ocassionally this can reveal problems on the array. See this [gallery](http://plmimagegallery.bmbolstad.com/) of interesting examples.
...
This method applies a model based background adjustment followed by quantile normalisation and a robust summary method (median polish) on the log$_2$ PM intensities to obtain probeset summary values.
