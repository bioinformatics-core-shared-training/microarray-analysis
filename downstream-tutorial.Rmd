...
We will log$_2$ transform the expression data
...
tion will obscure any biological differences that exist. As we saw yesterday, the `genefilter` package contains a suite
...
- Note that the matrix has some values that appear to be missing
...
the Euclidean. As we saw above, the `cor` function can be used to calculate the correlation of columns in a matrix. Each row
...
- However, the rows are named according to the row names in the expression matrix
    ...
    + also, we have sample names without any information of the sample groupings.
- The column labels can be changed by specifying the `labCol` argument    
...
In a similar way to adding colours to a dendrogram

```{r}
sampleColours <- matrix(nrow=length(SampleGroup),ncol=1)
sampleColours[,1] <- as.character(groupColours)
heatmap (as.matrix(exprs(colonData)[highVarGenes, ]),
         labCol = SampleGroup,
         ColSideColors = sampleColours)
...
It is particularly useful to overlay known sample attributes to the plot which explain the patterns.
...
Patients <- pd$characteristics_ch1.1
text(pca$rotation[,1],pca$rotation[,2]-0.01,labels = Patients)
```
