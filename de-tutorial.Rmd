...
******
## Q. Create a data frame containing genes with i) adjsuted p-value < 0.05 ii) log-fold change > 1. How many genes does this contain?
## Q. Now order this data frame according to chromosome

******

```{r echo=FALSE}
filtResults <- testResults[testResults$Chromosome != "" & testResults$adj.P.Val < 0.05 & testResults$logFC < 1,]
dim(filtResults)
head(filtResults)

orderedHits <- testResults[order(testResults$Chromosome),]
head(orderedHits)
```
...
Patient <- gsub("patient: ","",Patient)
design.better <- model.matrix(~0+Patient+SampleGroup)
```
...
```{r}
fit.better <- lmFit(exprs(gse.expFilt), design.better)
fit2.better <- eBayes(fit.better)
fit2.better$genes <- anno
topTable(fit2.better,coef=10)
table(decideTests(fit2.better[,10]))
sum(abs(decideTests(fit2.better[,10]))==1)
